...
description: '## 1. Назначение  Документ описывает универсальный формат **контейнера
  HMP**, применяемый для передачи и хранения всех типов данных внутри сети HyperCortex
  Mesh Protocol. Контейнеры служат стандартной...'
type: Article
tags:
- HMP
- REPL
...
```json
{
  ...
  "description": "# 🧩 HMP Container Specification (v1.2-draft)  ## 1. Назначение  Документ описывает универсальный фор..."
}
```
