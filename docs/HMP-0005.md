...
The revocation announcement MUST be signed with the (now compromised) private key
...
Agents MAY include a `mailman` list in the `payload` block inside `peer_announce`,
representing intermediate relay nodes that are willing to forward direct messages
...
**Group DIDs (virtual agents)**

The protocol allows the use of DIDs whose private key is jointly controlled by multiple participants.
Such a “virtual agent”:

* has a single DID and a single public key;
* can receive encrypted containers addressed to this DID;
* can publish containers on behalf of the group using the shared private key.

This enables multicast-style communication through a shared agent identifier, without modifying the container format or encryption mechanisms.
